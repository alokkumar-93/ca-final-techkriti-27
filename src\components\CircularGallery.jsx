...
const DEFAULT_FONT = 'bold 28px Figtree';
...
function createTextTexture(gl, text, font = 'bold 28px monospace', color = 'black') {
...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = '28px sans-serif' }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class App {
  ...
  createRenderer() {
    ...
    this.gl.clearColor(0.02, 0.04, 0.09, 1.0);
  ...
  createGeometry() {
    ...
    this.planeGeometry = new Plane(this.gl, {
      heightSegments: isMobile ? 6 : 12,
      widthSegments: isMobile ? 12 : 24
    });
  }
  ...
  onTouchMove(e) {
    ...
    // Track touch flick velocity
    this.touchVelocity = (this.lastTouchX - x);
    this.lastTouchX = x;

    const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
    // Higher touch sensitivity multiplier for light, effortless swiping on phones!
    const multiplier = isMobile ? 0.055 : 0.035;
  ...
  onTouchUp(e) {
    ...
    // Apply smooth momentum flick on mobile swipe!
    if (Math.abs(this.touchVelocity) > 2) {
      this.scroll.target += this.touchVelocity * 0.15;
    }
    ...
    const dx = Math.abs(endX - (this.start || 0));
    const dy = Math.abs(endY - (this.startY || 0));

    // Intentional tap check: dx < 12px AND dy < 12px AND duration < 400ms!
    if (dx < 12 && dy < 12 && duration < 400 && this.onItemClick && this.medias && this.medias.length && this.gl && this.gl.canvas) {
      const rect = this.gl.canvas.getBoundingClientRect();
      const clickX = ((endX - rect.left) / rect.width) * 2 - 1;
      const clickY = -(((endY - rect.top) / rect.height) * 2 - 1);

      // Convert NDC click coordinates to 3D world space
      const worldX = clickX * (this.viewport.width / 2);
      const worldY = clickY * (this.viewport.height / 2);
      ...
      for (let i = 0; i < this.medias.length; i++) {
        ...
        // Check if click position falls inside the 3D card bounding box
        if (
          worldX >= posX - halfWidth &&
          worldX <= posX + halfWidth &&
          worldY >= posY - halfHeight &&
          worldY <= posY + halfHeight
        ) {
          clickedMedia = media;
          break; // Exact hit found!
        }

        // Fallback distance calculation to target position (worldX, worldY)
        const dist = Math.hypot(posX - worldX, posY - worldY);
        if (dist < minDistance) {
          minDistance = dist;
          clickedMedia = media;
        }
      ...
      if (clickedMedia && clickedMedia.image) {
        this.onItemClick(clickedMedia.image);
      }
    }
  }
  onWheel(e) {
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular image gallery. Drag right or left to rotate."
    />
  );
}
